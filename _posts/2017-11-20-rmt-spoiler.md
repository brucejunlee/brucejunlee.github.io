...
量子理论告诉我们，atomic nuclei有许多分立的能级间隔不均匀，像一部梯子。计算每个梯级之间的间距，你需要知道的无数可能的核可以跳的方式从一个到另一个，和那些事件发生的概率。维格纳不知道，所以他随意挑选概率的数字，把它们排列成一个方阵叫做矩阵。
...
矩阵是一个整洁的方式来表达不同层次之间的许多联系。它还允许维格纳利用矩阵的强大数学，以便对能量水平作出预测。
...
奇怪的是，他发现这个简单的方法使得他的作品，任何一级将附近其他的可能性，在任何真正的知识的缺失。维格纳的研究成果，用几行代数计算出来，远比任何人预期的要有用得多，而且在接下来的几年里的实验显示出与他的预测非常接近的程度。尽管如此，他们为什么工作至今仍是个谜。
...
最值得注意的是，从那时起维格纳的思想是如何被使用的。它可以应用于许多涉及许多相互关联的变量的问题，这些变量的连接可以表示为一个随机矩阵。
...
维格纳的思想和一些核物理完全无关的之间关系的第一次发现偶然的相遇在上世纪70年代初英国物理学家戴森和美国数学家Hugh Montgomery在。
...
Montgomery一直在探索数学中最著名的函数之一，即黎曼ζ函数，它是寻找质数的关键。这些数字，如2, 3, 5和7，只能由它们自己和1整除。它们在数学中占有特殊的地位，因为每一个大于1的整数都可以从它们中建立起来。
...
1859，一位名叫Bernhard Riemann的德国数学家推测了一个关于zeta函数零点的简单规则。零点与素数的分布密切相关。
...
数学家们从未能够证明黎曼的假设。Montgomery也不能，但他已经找到了一个公式来寻找一个零的可能性，如果你已经知道附近的另一个位置。当Montgomery告诉Dyson这个公式，物理学家立即意识到这是非常相同的人，维格纳设计了核能量水平。
...
直到今天，没有人知道为什么质数应该与维格纳的随机矩阵有关，更不用说核能了。但这一联系是明确的。明尼阿波利斯明尼苏达大学的数学家Andrew Odlyzko计算多达1023个零点的黎曼zeta函数的位置发现一个近乎完美的协议与随机矩阵理论。
...
随机矩阵理论的奇特描述力并没有就此止步。在过去的十年里，它证明了它特别擅长描述一系列混乱的物理系统。
...
**拆台**：最近，例如，在哈佛大学的物理学家Ferdinand Kuemmeth和他的同事们用它来预测的能量水平的电子在纳米金他们的构造。
...
传统的理论认为，这种能量水平应该受到各种因素的影响，包括纳米颗粒的精确形状和大小以及原子的相对位置，这些原子被认为或多或少是随机的。然而，Kuemmeth的团队发现，随机矩阵理论描述很准确的测量水平（arXiv. org / ABS / 809.0670）。
...
一个物理学家团队由雷根斯堡大学的Jack Kuipers在德国发现了同样强大的协议在电子的奇特的行为跳跃混乱在量子点-基本上是一个小盒子可以捕获和保持单量子粒子（物理评论快报，vol 104，p 027001）。
...
列表中已发展到令人难以置信的地步，从量子引力和量子色动力学晶体的弹性性质。”从随机矩阵理论中产生的规律，对几乎所有的量子系统都具有普遍有效性。这是一个惊人的事实，说：”物理学家Thomas Guhr在瑞典技术学院的隆德。
...
随机矩阵理论得到了纽约大学的数学家喜欢想象，可能会有更多的一般模式，也有Percy Deift。”这种思维在数学中并不常见，”他说。数学家们往往认为他们的每一个问题都有其独特的特点。但近年来，我们开始发现，来自不同领域的问题，往往没有明显的联系，都表现得非常相似。”
...
例如，在2006的一篇论文中，他展示了随机矩阵理论是如何很自然地应用到某些纸牌游戏的数学中，如公交车在城市中聚集的方式，以及分子在气体中弹跳的轨迹等。
...
也许最重要的问题是，物理学和数学背后是否有一些深奥的理论解释了为什么随机矩阵似乎捕捉到了关于现实的基本真理。一定是有原因的，但我们还不知道它是什么，”Nadakuditi承认。同时，随机矩阵理论已经改变了我们如何看待随机系统，并试图了解它们的行为。它可能提供了一个新的工具，例如，检测全球气候的微小变化。
...
早在1991，国际科学合作就开展了被称为“赫德岛”的可行性试验。由于声音通过世界海洋的传播可能对气温上升提供了一个敏感的测试，他们在印度洋附近听到了一个巨大的嗡嗡声，并在世界各地使用了一系列传感器来接收它。
...
20年后重复这个实验可能会产生关于气候变化的宝贵信息。但是，对于声音对当地海洋生物的有害影响的担忧意味着今天的实验必须用微弱的信号来执行，而这些信号是微弱的，无法用普通手段探测到。这就是随机矩阵理论的来源。
...
在过去的几年中，Nadakuditi与麻省理工学院的Alan Edelman和其他人的工作，开发了一种基于随机矩阵的信号检测理论。它特别适合于在全球部署的大量传感器的操作。我们发现，你可以用极其微弱的声音和原则仍希望能探测到的信号，”Nadakuditi说。
...
另一些人则使用随机矩阵理论来做令人惊奇的事情，例如使光线穿过明显不可穿透的不透明材料。去年，物理学家Allard Mosk在荷兰的屯特大学和他的同事们用它来描述统计之间的联系，光线落在物体上，光被散射掉。对于一个很好散射光线的不透明物体，他指出，这些连接可以用完全随机矩阵来描述。
...
出现的是一些其他分析没有提出的奇怪可能性。矩阵显示，应该有莫斯克所说的“开放通道”的具体类型的波，而不是被反射，会穿过材料。事实上，当莫斯克的团队光照与精心构造的波前通过厚，氧化锌涂料不透明层，他们看到了一个在光传输急剧增加。
...
尽管如此，随机矩阵理论的最引人注目的应用可能还没有到来。”一些主要结果已经几十年了，”说，物理学家Jean Philippe Bouchaud在巴黎的É科尔法国理工学院，但是他们突然变得更加重要在科学等许多领域的数据集的处理。”
...
从粒子物理学和天文学到生态学和经济学，收集和处理大量数据已经司空见惯。经济学家可能会筛选出数百个数据集，以寻找能解释通货膨胀变化的东西——也许是石油期货、利率或工业库存。像Amazon.com这样的企业依靠类似的技术来发现购买者的行为模式，并帮助指导他们的广告。
...
虽然随机矩阵理论表明这是一种很有前途的方法，但它也指出了隐藏的危险。随着越来越多的复杂数据被收集，所研究的变量的数量不断增长，它们之间明显的相关性的数量增长得更快。有足够的变量进行测试，几乎可以肯定的是，你会发现看起来很重要的相关性，即使它们不是。
...
**拆台**：假设你在许多经济指数上有多年的数据，包括通货膨胀、就业和股票市场价格。你要寻找他们之间的因果关系。Bouchaud和他的同事们已经证明，即使这些变量都是随机波动，最大的相关性将大到足以显著。
...
这就是所谓的“维数灾难”。这意味着，虽然大量的信息使学习一切都很容易，但它也很容易找到无意义的模式。这就是随机矩阵的方法，把有意义的和不正确的东西分开。
...
在20世纪60年代末，数学家Vladimir Marcenko和乌克兰Leonid牧草来源的描述非常大的关键性的一个基本的数学结果，随机矩阵。他们的结果允许你计算你应该期望找到的数据集之间的关联度。这使得区分真正的特殊情况和偶然事故成为可能。这些关联的强度相当于维格纳原著中的核能水平。
...
Bouchaud的团队现在已经表明，这一想法投在许多经济预测的可信度表示怀疑，尤其是那些自称看今后的几个月。当然，这样的预测是经济体制的基础。但是我们能相信他们吗？
...
发现，Bouchaud和他的同事们如何看待美国的通货膨胀率可以通过广泛的经济指标解释，如工业生产、零售销售、消费者和生产者的信心，利率和石油价格。
...
使用从1983到2005的数字，他们首先计算了所有可能的数据相关性。他们发现了什么似乎是很重要的结果——一个明显的模式显示了经济指标的变化如何导致下一次通货膨胀的变化。在粗心的观察者，这使它看起来好像可以有信心地预测通货膨胀。
...
但当Bouchaud的团队将Marcenko和牧草的数学，他们得到了一个惊喜。他们发现只有少数这些明显的相关性可以被认为是真实的，从某种意义上说，它们确实与预期的单独存在密切相关。他们的调查结果表明，通货膨胀只有提前一个月才能预料到。展望未来两个月，数学显示没有任何可预测性。添加更多的数据就不会导致更多的可预测性，一些经济学家希望，”Bouchaud说。
...
近年来，一些经济学家开始对大量数据所作的预测表示怀疑，但他们是少数。大多数人认为更多的测量意味着更好的预测能力。这可能是一种错觉，随机矩阵理论可能是区分真实与不真实的工具。
